/* Generated by openApiCatalogGenerate. Do not edit. */
export const leadsAndContactsGetDefinition = {
  description: "Finds emails, social links, phones, and other contacts from websites.",
  folder: "get-leads-and-contacts",
  method: "GET",
  operationId: "getLeads-And-Contacts",
  parameters: [
    {
      description:
        "The parameter defines the way you want to submit your task to Outscraper. It can be set to `false` to open an HTTP connection and keep it open until you get your results, or `true` (default) to just submit your requests to Outscraper and retrieve them later (usually within 1-3 minutes) with the [Request Results](#tag/requests/GET/requests/{requestId}) endpoint. Each response is available for `4` hours after a request has been completed.\n\nA good practice is to send async requests and start checking the results after the estimated execution time. Check out [this](https://github.com/outscraper/outscraper-python/blob/master/examples/Run%20Async%20Requests.md) Python implementation as an example.\n\nAs most of the requests take some time to be executed the `async=true` option is preferred to avoid HTTP request timeouts.",
      example: false,
      in: "query",
      location: "query",
      name: "async",
      required: true,
      schema: {
        default: true,
        type: "boolean",
      },
    },
    {
      description: "The parameter specifies the number of Contacts per one company.",
      in: "query",
      location: "query",
      name: "contactsPerCompany",
      required: false,
      schema: {
        default: 3,
        type: "integer",
      },
    },
    {
      description: "The parameter specifies the number of email addresses per one contact.",
      in: "query",
      location: "query",
      name: "emailsPerContact",
      required: false,
      schema: {
        default: 1,
        type: "integer",
      },
    },
    {
      description:
        "The parameter defines which fields you want to include with each item returned in the response. By default, it returns all fields. Use `&fields=query,name` to return only the specific ones.",
      in: "query",
      location: "query",
      name: "fields",
      required: false,
      schema: {
        type: "string",
      },
    },
    {
      description:
        "The parameter defines the format of the output data. Using this parameter overwrites the `ui` parameter to `true`.",
      in: "query",
      location: "query",
      name: "format",
      required: false,
      schema: {
        collectionFormat: "csv",
        items: {
          default: "xlsx",
          enum: ["xlsx", "csv", "json", "parquet"],
          type: "string",
        },
        type: "array",
      },
    },
    {
      description:
        "The parameter specifies whether to include only general email (info@, support@, etc.) or only not general email (paul@, john@, etc.).",
      in: "query",
      location: "query",
      name: "generalEmails",
      required: false,
      schema: {
        default: null,
        type: "boolean",
      },
    },
    {
      description:
        "Preferred contacts. It supports multiple categories (e.g., `preferredContacts=technical&preferredContacts=influencers`).",
      in: "query",
      location: "query",
      name: "preferredContacts",
      required: false,
      schema: {
        collectionFormat: "csv",
        items: {
          enum: [
            "decision makers",
            "influencers",
            "procurement/purchasing",
            "technical",
            "finance",
            "operations",
            "marketing",
            "sales",
            "maintenance",
            "human resources",
            "legal and compliance",
            "supply chain/logistics",
            "education/training",
          ],
          type: "string",
        },
        type: "array",
      },
    },
    {
      description:
        "Domain or link (e.g., `outscraper.com`).\n\nIt supports batching by sending arrays with up to `1000` queries (e.g., `query=text1&query=text2&query=text3`). It allows multiple queries to be sent in one request and to save on network latency time.",
      in: "query",
      location: "query",
      name: "query",
      required: true,
      schema: {
        collectionFormat: "csv",
        items: {
          type: "string",
        },
        type: "array",
      },
    },
    {
      description: "The parameter specifies the number of contacts to skip. Used for pagination.",
      in: "query",
      location: "query",
      name: "skipContacts",
      required: false,
      schema: {
        default: 3,
        type: "integer",
      },
    },
    {
      description:
        "The parameter defines whether a task will be executed as a UI task. This is commonly used when you want to create a regular platform task with API.\n\nUsing this parameter overwrites the `async` parameter to `true`.",
      in: "query",
      location: "query",
      name: "ui",
      required: false,
      schema: {
        default: false,
        type: "boolean",
      },
    },
    {
      description:
        "The parameter defines the URL address (callback) to which Outscraper will create a POST request with a JSON body once a task/request is finished.\n\nUsing this parameter overwrites the webhook from integrations.",
      in: "query",
      location: "query",
      name: "webhook",
      required: false,
      schema: {
        type: "string",
      },
    },
  ],
  path: "/leads-and-contacts",
  responses: [
    {
      content: [
        {
          example: {
            data: [
              {
                contacts: [
                  {
                    categories: ["technical"],
                    emails: [
                      {
                        source: "linkedin",
                        value: "vlad.sima@outscraper.com",
                      },
                    ],
                    full_name: "Vlad Sima",
                    socials: {
                      linkedin: "simokhodsky",
                    },
                    title: "data alchemist",
                  },
                  {
                    emails: [
                      {
                        source: "smtp",
                        value: "kseniia@outscraper.com",
                      },
                    ],
                    full_name: "kseniia maiesh",
                  },
                  {
                    emails: [
                      {
                        first_seen: "08/12/2025 23:10:06",
                        last_seen: "08/13/2025 01:06:10",
                        source: "smtp",
                        value: "vlad@outscraper.com",
                      },
                    ],
                    full_name: "vlad may",
                  },
                ],
                details: {
                  address: "Delaware state",
                  city: "Middletown",
                  country: "US",
                  employees: "<25",
                  founded: "2018",
                  industry: ["Custom Software & IT Services", "Business Services"],
                  name: "Outscraper",
                  postal_code: "19709",
                  sales: "< $5M",
                  size: {
                    f: 11,
                    t: 50,
                  },
                  state: "Texas",
                  type: "Private",
                },
                domain: "outscraper.com",
                emails: [],
                phones: [
                  {
                    source: "https://www.facebook.com/outscraper/",
                    value: "12812368208",
                  },
                ],
                query: "outscraper.com",
                site_data: {
                  description:
                    "Scrape Google Maps Places, Business Reviews, Photos, Play Market Reviews, and more. Get any public data from the internet by applying cutting-edge technologies.",
                  generator: "All in One SEO Pro (AIOSEO) 4.7.0",
                  has_fb_pixel: false,
                  has_google_tag: false,
                  keywords: "data extraction, web scraping, Google Maps, lead generation",
                  title: "Outscraper - get any public data from the internet",
                },
                socials: {
                  crunchbase: "https://www.crunchbase.com/organization/outscraper",
                  discord: "https://discord.gg/kcQzJW7rYr",
                  facebook: "http://www.facebook.com/outscraper",
                  github: "https://github.com/outscraper",
                  linkedin: "http://www.linkedin.com/company/outscraper",
                  twitter: "http://www.twitter.com/outscraper",
                  whatsapp: "https://wa.me/12812368208",
                  youtube: "https://www.youtube.com/channel/UCDYOuXSEenLpt5tKNq-0l9Q",
                },
              },
            ],
            id: "your-request-id",
            status: "Success",
          },
          mediaType: "application/json",
          schema: {},
        },
      ],
      description: {},
      status: "200",
    },
    {
      content: [
        {
          example: {
            id: "your-request-id",
            results_location: "https://api.outscraper.cloud/requests/your-request-id",
            status: "Pending",
          },
          mediaType: "application/json",
          schema: {},
        },
      ],
      description:
        "The response contains a request ID that can be used to fetch results by using [Request Results](#tag/requests/GET/requests/{requestId}) endpoint. Each response is available for `4` hours after a request has been completed.",
      status: "202",
    },
    {
      content: [
        {
          example: {
            error: true,
            errorMessage:
              "401 Unauthorized: The server could not verify that you are authorized to access the URL requested. You either supplied the wrong credentials (e.g. a bad password), or your browser doesn't understand how to supply the credentials required.",
          },
          mediaType: "application/json",
          schema: {},
        },
      ],
      description: "Wrong or missing API Key (token).",
      status: "401",
    },
    {
      content: [
        {
          example: {
            error: true,
            errorMessage: "Please close past due invoices or verify your card information is correct.",
          },
          mediaType: "application/json",
          schema: {},
        },
      ],
      description: "Past due invoices or a payment method not connected.",
      status: "402",
    },
    {
      content: [
        {
          example: {
            error: true,
            errorMessage:
              "422 Unprocessable Entity: The request was well-formed but was unable to be followed due to semantic errors.",
          },
          mediaType: "application/json",
          schema: {},
        },
      ],
      description: "Invalid query URL parameters.",
      status: "422",
    },
  ],
  security: [
    {
      ApiKeyAuth: [],
    },
  ],
  summary: "Leads & Contacts",
  tags: ["Email Related"],
  "x-codeSamples": [
    {
      lang: "cURL",
      source:
        'curl -X GET "https://api.outscraper.cloud/leads-and-contacts?query=outscraper.com&async=false" -H  "X-API-KEY: YOUR-API-KEY"',
    },
    {
      lang: "cURL (async)",
      source:
        'curl -X GET "https://api.outscraper.cloud/leads-and-contacts?query=outscraper.com" -H  "X-API-KEY: YOUR-API-KEY"',
    },
    {
      lang: "Python",
      source:
        "from outscraper import OutscraperClient\n\napi_client = OutscraperClient(api_key='YOUR-API-KEY')\nresults = api_client.contacts_and_leads(['outscraper.com'])",
    },
    {
      lang: "Ruby",
      source:
        "require 'Outscraper'\n\nclient = Outscraper::Client.new('SECRET_API_KEY')\nresults = client.contacts_and_leads('outscraper.com')",
    },
    {
      lang: "PHP",
      source:
        "$client = new OutscraperClient('YOUR-API-KEY');\n$results = $client->contacts_and_leads(['outscraper.com']);",
    },
    {
      lang: "Node.js",
      source:
        "const Outscraper = require('outscraper');\n\nlet client = new Outscraper('YOUR-API-KEY');\nclient.contactsAndLeads(['outscraper.com']).then(response => {\n   console.log(response);\n});",
    },
    {
      lang: "Go",
      source:
        'import "github.com/outscraper/outscraper-go"\n\nclient := outscraper.Client{ApiKey: "SECRET_API_KEY"}\nresults, _ := client.ContactsAndLeads(map[string]string {\n    "query": "outscraper.com",\n})',
    },
    {
      lang: "Java",
      source:
        'OutscraperClient client = new OutscraperClient("SECRET_API_KEY");\nJSONArray results = client.contactsAndLeads(new HashMap<String, Object>() {{\n    put("query", "outscraper.com");\n}});',
    },
  ],
} as const
